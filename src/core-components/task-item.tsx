        ...
        <Card size="md">
            {!isEditing ?
                ...
                <div className="flex items-center gap-4">
                   ...
                   {!loading ?  <Text className={cx("flex-1", { 'line-through': task?.concluded, })}>{task?.title}</Text> : <Skeleton className="h-6 flex-1" />}
                    <div className="flex gap-1">
                        <ButtonIcon icon={TrashIcon} variant="tertiary" type="button" onClick={handleDeleteTask} loading={loading} handling={isDeletingTask}/>
                        <ButtonIcon icon={PencilIcon} variant="tertiary" onClick={handleEditTask} type="button" loading={loading}/>
                    </div>
                </div>
